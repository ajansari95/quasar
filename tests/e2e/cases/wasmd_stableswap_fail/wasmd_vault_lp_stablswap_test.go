...
func (s *WasmdTestSuite) SetupSuite() {
	...
	s.deployPrimitives(ctx, s.ContractsDeploymentWallet, lpStrategyContractPath, "lp_strategy_test", init1)
...
func (s *WasmdTestSuite) TestLpStrategyContract_StableswapActions() {
	t := s.T()
	ctx := context.Background()
	...
	testCases := []struct {
		...
		},
		{
			...
			UnbondAmount:            "10000000",
			expectedNumberOfUnbonds: 1,
		},
		{
			Account:                  *accBondTest0,
			Action:                   "claim_fail",
			expectedBalanceChange:    10000000,
		...
		},
		{
			...
			Action:                  "unbond",
			BondAmount:              sdk.NewCoins(),
			UnbondAmount:            "5000000",
			expectedNumberOfUnbonds: 2,
		},
		{
			Account:                  *accBondTest0,
			Action:                   "claim",
			expectedBalanceChange:    15000000,
			expectedBalanceDeviation: 0.1,
		},
	}
	...
	for _, tc := range testCases {
		...
		case "claim_fail":
			...
			balanceChange := balanceAfter - balanceBefore
			s.Require().True(int64(0) == balanceChange)
		default:
