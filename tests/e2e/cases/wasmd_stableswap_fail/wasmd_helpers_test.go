...
func (s *WasmdTestSuite) deployPrimitives(ctx context.Context, acc *ibc.Wallet, filePath, label string, initArgs1 any) {
...
func (s *WasmdTestSuite) CreatePools(ctx context.Context) {
	...
	s.Require().NoError(err)
	s.CreatePoolOnOsmosis(ctx, s.Osmosis(), s.E2EBuilder.OsmosisAccounts.Treasury.KeyName, poolBz, "stableswap")
}
